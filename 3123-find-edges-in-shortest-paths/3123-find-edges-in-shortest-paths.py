class Solution:
    def findAnswer(self, num_nodes: int, edges: List[List[int]]) -> List[bool]:
        ...
        for node1, node2, weight in edges:
            if start_node_distances.get(node1, inf) + weight + end_node_distances.get(node2, inf) == shortest_distance \
                or start_node_distances.get(node2, inf) + weight + end_node_distances.get(node1, inf) == shortest_distance:
                answers.append(True)
            else:
